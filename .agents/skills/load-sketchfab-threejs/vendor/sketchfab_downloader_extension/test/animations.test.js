...
test("animationUrls accepts legacy 31-character clip ids", () => {
  const legacy = "040494ef4e4a4e9d91073e8a1bfca54";
  const info = {
    files: [{ osgjsUrl: "https://media.sketchfab.com/models/UID/HASH/files/F/file.binz" }],
    options: { animation: { order: [legacy] } },
  };
  assert.equal(legacy.length, 31);
  assert.equal(animationUrls(info)[0].uid, legacy);
});
