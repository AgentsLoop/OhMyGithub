...
deterministic checks first, pass the failed phase and attempt evidence to the
same issue-linked OpenCode session with the `build` command, and retry twice on
the same emulator. Treat OpenCode timeout exits as permission to continue
deterministic checks. After those attempts, provision one fresh emulator; hold
its final failure for interactive SSH diagnosis, and clean the temporary
`known_hosts` file only after the session ends. This is implemented in the Android verifier and
